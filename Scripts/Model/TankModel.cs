...
public class TankModel : PunBehaviour {
    ...
    public void Shoot()
    {
        ...
        if (now - timeOfLastShooting >= timeBetweenShooting)
        {
            ...
            PhotonNetwork.Instantiate("CannonBall", cannnonMouth.transform.position, cannnonMouth.transform.rotation, 0);
        }
    ...
    public void TakeDamage(int damage)
    {
        ...
        if(gameObject == GameManager.GetGMInstance().localTank)
            playDamageEffect();
        ...
        //{       //MasterClient调用
            ...
            if (health <= 0)
                health = 0;
            UpdateHP(health);
            //photonView.RPC("UpdateHP", PhotonTargets.All, health);  //更新所有客户端，该玩家对象的生命值
